...
int main(int argc, char *argv[])
{
    ...
    printf("===================================================================\n");
    printf("NetStream Example.  Copyright (c) Firelight Technologies 2004-2016.\n");
    printf("===================================================================\n\n");
    ...
    */
    ...
    /*
        Bump up the file buffer size a little bit for net streaming. Decode buffer is lef
t at default
    */
    result = FMOD_System_SetStreamBufferSize(system, 64*1024, FMOD_TIMEUNIT_RAWBYTES);
    ERRCHECK(result);

    result = FMOD_System_CreateSound(system, argv[1], FMOD_SOFTWARE | FMOD_2D | FMOD_CREATESTREAM | FMOD_NONBLOCKING,  0, &sound);
    ERRCHECK(result);
    ...
    */
    do
    {
        ...
        printf("\rTime %02d:%02d:%02d : %s : (%3d%%) %s     ", ms / 1000 / 60, ms / 1000 % 60, ms / 10 % 100, openstate == FMOD_OPENSTATE_BUFFERING ? "Buffering..." : openstate == FMOD_OPENSTATE_CONNECTING ? "Connecting..." : paused ? "Paused       " : playing ? "Playing      " : "Stopped      ", percent, starving ? "STARVING" : "        ");
        fflush(stdout);
